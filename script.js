...
hadiths.forEach(h => {
  ...
  card.innerHTML = `
    <blockquote class="hadith-text">
      ...
      ${isLong ? `<span class="hadith-full" style="display:none;">${h.text}</span>` : ''}
    </blockquote>
    <div class="hadith-meta">
      <p><span class="bold">خلاصة حكم المحدث:</span> 
        <span class="judgment-badge ${h.ruling.includes('صحيح') ? 'judgment-sahih' :
      h.ruling.includes('حسن') ? 'judgment-hasan' : 'judgment-daif'
    }">${h.ruling}</span>
      </p>
...
document.querySelectorAll('.toggle-hadith').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const text = card.querySelector('.hadith-full')?.innerText || card.querySelector('.hadith-preview').innerText;
    const narrator = card.querySelector('.hadith-meta p:nth-child(2)').innerText.replace('الراوي:', '').trim();
    const compiler = card.querySelector('.hadith-meta p:nth-child(3)').innerText.replace('المحدث:', '').trim();
    const source = card.querySelector('.hadith-meta p:nth-child(4)').innerText.replace('المصدر:', '').trim();

    document.getElementById('popupHadith').innerText = text;
    document.getElementById('popupReference').innerText = `رواه ${narrator}, ${compiler}, ${source}`;

    document.getElementById('hadithPopup').style.display = 'flex';
  });
});

document.getElementById('closePopup').addEventListener('click', () => {
  document.getElementById('hadithPopup').style.display = 'none';
});

document.getElementById('hadithPopup').addEventListener('click', (e) => {
  if (e.target === document.getElementById('hadithPopup')) {
    document.getElementById('hadithPopup').style.display = 'none';
  }
});